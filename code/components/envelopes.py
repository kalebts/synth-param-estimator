...
class ADSREnvelope:
    ...
    def __init__(self, attack_duration=0.05, decay_duration=0.2, sustain_level=0.7, \
                 sustain_duration = 2, release_duration=0.3, sample_rate=44_100):
        """
    ...
    def _get_ads_stepper(self):
        ...
        if self.sustain_duration > 0:
            steppers.append(iter([self.sustain_level] * math.ceil((self.sustain_duration  * self._sample_rate))))   
        if self.release_duration > 0:
            steppers.append(itertools.count(start=self.sustain_level, \
            step= - self.sustain_level / (self.release_duration * self._sample_rate)))
            # steppers.append(iter([0] * math.ceil((20  * self._sample_rate * 2))))
        steppers.append(iter([0] * math.ceil((20  * self._sample_rate * 2))))
        while True:
            l = len(steppers)
            if l >= 4:
                val = next(steppers[0])
                if l == 5 and val > 1: # for attack
                    # print('attack')
                    steppers.pop(0)
                    val = next(steppers[0])
                elif l == 4 and val < self.sustain_level: # decay
                    # print('decay')
                    steppers.pop(0)
                    val = next(steppers[0])
            elif l >= 1:
