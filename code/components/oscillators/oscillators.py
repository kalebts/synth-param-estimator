...
class CustomOscillator(Oscillator):
    def __init__(self, freq=440, phase=0, amp=1, \
    ...
    def _post_freq_set(self):
        self._step = (compile(self.formula) * self._f) / self._sample_rate 
    ...
    def __next__(self):
        x = self._i
        # val = math.sin(self._i + self._p) # maybe incorporate phase
        val = compile(self.formula)(self._i + self._p) # maybe incorporate phase
        self._i = self._i + self._step
        if val < self.threshold:
            val = self._wave_range[0]
        else:
